...
// Import JSON data
const healthCentersData = require('./data/list_of_health_centers.json') as HealthCenterData[];
const medicinesData = require('./data/medicines.json') as MedicineData;
const diseaseMedicinesData = require('./data/disease_medicines.json') as DiseaseMedicines;
...
const MapComponent: React.FC<MapComponentProps> = ({
  ...
  // Show a simple view with health centers list instead of map
  return (
    <View style={[style, { backgroundColor: '#f0f0f0', padding: 10 }]}>
      ...
      <ScrollView style={{ maxHeight: 300 }}>
        {markers.map((marker) => (
          ...
          >
            <Text style={{ fontWeight: 'bold', fontSize: 14 }}>{marker.name}</Text>
            <Text style={{ color: '#666', fontSize: 12, marginTop: 4 }}>
              Medicines: {marker.availableMedicines.slice(0, 3).join(", ")}
              {marker.availableMedicines.length > 3 && "..."}
            </Text>
            <TouchableOpacity
              style={{
              ...
              onPress={() => openOSMSearch(marker.latitude, marker.longitude)}
            >
